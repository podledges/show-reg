...
const config: Config = { version: 1, target: "MCXC444", manual: process.env.SHOW_ME_TEST_MANUAL ?? "manual.pdf", pdftotext: "pdftotext", model: "current", preference: "accuracy" };
...
test("config survives restart, replaces cleanly, validates and rejects corrupt data", async () => {
  const directory = await mkdtemp(join(tmpdir(), "show-me-config-"));
  try {
    ...
    assert.deepEqual(await readdir(join(directory, ".pi")), ["show-me.json"]);
    assert.throws(() => validateConfig({ ...config, version: 2 }));
    assert.throws(() => validateConfig({ ...config, manual: "" }));
    await writeFile(join(directory, ".pi/show-me.json"), "broken");
...
test("process arguments are literal, not shell input", async () => {
  ...
  await assert.rejects(runText("show-me-no-such-executable", []), /extraction failed/);
...
test("real manual: different peripherals, title aliases and bounded source", async (t) => {
  if (!process.env.SHOW_ME_TEST_MANUAL) { t.skip("Set SHOW_ME_TEST_MANUAL to the MCX-C44X reference PDF for this optional integration test."); return; }
...
test("Pi extension loads; mocks verify no-call failures, isolated request and identity", async (t) => {
  ...
  const loaded = await loadExtensions([join(root, "extensions/show-me/index.ts")], root);
  assert.deepEqual(loaded.errors, []);
  const commands = loaded.extensions[0].commands;
  assert.deepEqual([...commands.keys()].sort(), ["show-me", "show-me-config"]);
  if (!process.env.SHOW_ME_TEST_MANUAL) { t.diagnostic("Pi command loading passed; set SHOW_ME_TEST_MANUAL to also exercise real-PDF model mocks."); return; }
  const directory = await mkdtemp(join(tmpdir(), "show-me-command-"));
  const messages: any[] = [];
  ...
  try {
    ...
    await commands.get("show-me").handler("MCG_C1", ctx);
    assert.match(messages.at(-1).content, /show-me-config/);
    assert.equal(calls, 0);
    await saveConfig(directory, { ...config, manual: resolve(root, config.manual) });
    await commands.get("show-me").handler("MCG_C9", ctx);
    ...
    await commands.get("show-me-config").handler("", ctx);
    ...
    const pending = commands.get("show-me").handler("MCG_C1", ctx);
    await started;
    await commands.get("show-me").handler("cancel", ctx);
    await pending;
